...
while True:
    ...
    img = cv.imread('../../Images/fig_geo.jpg')
    imgCopy = img.copy()
    ...
    for i, cont in enumerate(sorted_contour):
        ...
        cv.putText(img, "Points : " + str(len(approx)),(x + w + 5, y + 20),cv.FONT_HERSHEY_COMPLEX, .5, (0.255, 0), 2)
        cv.putText(img, "Area : " + str(int(area)),(x + w + 5, y + 45),cv.FONT_HERSHEY_COMPLEX, .5, (0.255, 0), 2)
